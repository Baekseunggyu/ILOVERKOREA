		...
		<div id="formContent">
			...
			<form name="signupForm" method="post" action="update">
				<div class="fadeIn first">
					...
					<div class="id">
						<input type="hidden" name="id" onchange="changeId()"
				...
				<div class="fadeIn second">
					<h6>비밀번호</h6>
					<input type="password" name="passwd" placeholder="영문숫자조합 8자리이상"
						required>
				</div>
				...
				<div class="fadeIn third">
					...
					<input type="password" name="passwd_check"
						placeholder="비밀번호를 일치시켜주세요" required>
				</div>
				...
				<div class="fadeIn sixth">
					<h6>전화번호</h6>
					<input type="text" name="tel" placeholder="예시 : 010-1234-5678"
						maxlength="13" value="${item.tel}">
				</div>
				...
				<div class="fadeIn seventh">
					...
					<div class="input-group">
						<input type="text" class="form-control form-control-sm"
							style="margin-left: 40px; margin-right: 10px;" name="email1"
							onfocus="this.value='';"> <input type="text"
							class="form-control form-control-sm" style="margin-right: 10px;"
							name="email2" value="" disabled> <select name="email3"
							style="margin-right: 40px; width: 60px;"
							onchange="email_change()" class="form-select form-select-sm">
							<option value="0">선택하세요</option>
							<option value="9">직접입력</option>
							<option value="@naver.com">@naver.com</option>
							<option value="@gmail.com">@gmail.com</option>
							<option value="@daum.net">@daum.net</option>
						</select> <input type="hidden" name="email" value="${item.email}">
					</div>
				</div>

				<div class="btn-group mt-1" style="float: right;">
					<button type="button" id="sign" class="btn btn-outline-primary  "
						style="margin-right: 10px; border-radius: 5px" onclick="signup()">수정</button>
					<button type="button" id="cancil" class="btn btn-outline-primary "
						style="margin-right: 10px; border-radius: 5px" onclick="goBack()">취소</button>
					<a class="btn btn-outline-danger bi bi-x-octagon-fill "
						style="border-radius: 5px; float: right;"
						href="delete?id=${item.id}">회원탈퇴</a>
				</div>
			</form>
		</div>
	</div>
