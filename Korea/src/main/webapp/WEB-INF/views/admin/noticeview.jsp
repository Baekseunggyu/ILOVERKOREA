								...
								<c:if test="${sessionScope.member.id == 'admin'}">
									<div style="width: 650px; text-align: center;">
										...
										<input type="button" value="수정"
											onclick="location.href = '/admin/noticeupdate?noticeId=${item.noticeId}'"
											class="btn btn-secondary btn-sm mb-1">
										<button type="button"
											onclick='location.href = "/admin/noticedelete?noticeId=${item.noticeId}"'
											class="btn btn-danger btn-sm mb-1">삭제</button>
										<button type="button" onclick="location.href='../'"
											class="btn btn-outline-success btn-sm mb-1">이전</button>
									</div>
								</c:if>
							</div>
						</div>
