								...
								<form method="post" enctype="multipart/form-data">
									<div>
										<a type="button" class="btn btn-outline-secondary btn-sm"
											href="/admin/tourlist"
											style="float: right; margin-bottom: 5px;">목록</a>
										<button class="btn btn-outline-success btn-sm"
											style="float: right; margin-bottom: 5px; margin-right: 5px;">변경</button>
									</div>
									<div>
									...
									<div class="form-group">
										<label>관광지명</label> <input type="text" name="tourName"
											value="${item.tourName}" class="form-control">
									</div>

									<div class="form-group row">
										<div class="col-6">
											<label>위도</label> <input type="text" name="tourMapx"
												value="${item.tourMapx}" class="form-control">
										</div>
										<div class="col-6">
											<label>경도</label> <input type="text" name="tourMapy"
												value="${item.tourMapy}" class="form-control">
										</div>
									</div>

									<div class="form-group row">
										<div class="col-6">
											<label>지역명</label> <select name="areaId" class="form-control">
									...
									<form id="uploadForm">
										<div class="input-group">
											<input type="file" name="uploadFile" class="form-control"
												id="inputGroupFile04">
											<button id="upload" class="btn btn-outline-secondary"
												type="button">이미지 추가</button>
										</div>
									</form>
									<ul id="images"
										style="list-style: none; list-style-type: none;">
										<c:forEach var="item" items="${item.tourImages}">
											<li><img src="/upload/files/${item.filename}"
												width="250px;">
												<button class="delete" data-id="${item.tourImageId}">삭제</button></li>
										</c:forEach>
										<c:if
											test="${item.tourImages == null || item.tourImages.size() < 1}">
